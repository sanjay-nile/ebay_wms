    ...
    <div class="content-wrapper">
            ...
            <div class="row">
                <div class="col-md-8 align-self-left">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">eBay Pending List</li>
                    </ol>
                </div>
            </div>
        </div>
        <div class="row">
                                ...
                                <form action="" method="get" class="form-horizontal" autocomplete="off" id="filter-frm">
                                    <div class="form-row align-items-center">
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <a href="{{ route('admin.pending.item') }}" class="btn btn-Reset">Reset</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Pending Lists</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <table class="table table-striped table-bordered table-hover admin-data-table admin-data-list table-sm">
                                    ...
                                    <tbody>
                                        @forelse($orders as $row)
                                            <tr>
                                                {{-- @if($row->order_status == 'IS-02' && !$row->existsMeta('inventory_status')) --}}
                                                    <td style="text-align: center;"><input name="order_ids[]" value="{{ $row->item_id }}" type="checkbox" class="selectone" /></td>
                                                {{-- @else
                                                    <td></td>
                                                @endif --}}
                                                ...
                                                <td class="ws">
                                                    @if(isset($row->common_error) && !empty($row->common_error))
                                                        {{ $row->common_error ?? '' }}
                                                    @else
                                                        @php
                                                            if(isset($row->publish_offer_response) && !empty($row->publish_offer_response)){
                                                                $error = json_decode($row->publish_offer_response);
                                                                echo $error->errors[0]->message ?? '';
                                                            } elseif(isset($row->create_offer_error) && !empty($row->create_offer_error)){
                                                                $error = json_decode($row->create_offer_error);
                                                                echo $error->errors[0]->message ?? '';
                                                            }
                                                        @endphp
                                                    @endif
