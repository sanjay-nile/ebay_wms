...
<body id="order_invoice">
    ...
    <div class="order_invoice-image-section">
        <div id="myowlSlides" class="owl-carousel owl-theme">
            @php
                $scan = json_decode(get_post_extra($package->id, 'scan_in_images'));
                $dis = json_decode(get_post_extra($package->id, 'dispatch_images'));
                $web = json_decode(get_post_extra($package->id, 'dispatch_webcam_image'));
                if(!empty($scan) && !empty($dis)){
                    $item = array_merge($scan, $dis);
                } elseif(!empty($scan)){
                    $item = $scan;
                } elseif(!empty($dis)){
                    $item = $dis;
            ...
            @forelse($item as $k)
                <div class="item">
                    <div class="order_invoice-image"><img src="{{ asset('public/uploads/'.$k) }}"></div>
                </div>
            @empty
            @endforelse
            @endif
