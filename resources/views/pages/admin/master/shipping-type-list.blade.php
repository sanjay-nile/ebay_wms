        ...
        <div class="we-page-title">
            <div class="row">
                <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
                    <h3 class="content-header-title mb-0 d-inline-block">Shipment Type</h3>
                    <div class="row breadcrumbs-top d-inline-block">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ getDashboardUrl()['dashboard'] }}">Home</a></li>
                                <li class="breadcrumb-item active">Shipment Type List</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    ...
                                    @forelse($shipping_list as $shipping)
                                        <tr>
                                            <td>{{ $i++ }}</td>
                                            <td>{{ $shipping->name }}</td>
                                            <td><span class="badge badge-{{ ($shipping->status==1)?'success':'danger' }}">{{ ($shipping->status==1)?"Active":"InActive" }}</span></td>
                                            <td>
                                                <button class="btn btn-view" data-id="{{ $shipping->id }}" data-status="{{ $shipping->status }}" data-name="{{ $shipping->name }}"><i class="la la-edit"></i></button>
                                                <a class="btn btn-delete btn-danger" onclick="return confirm('Are you sure you want to delete this Shipment?')" href="{{route('admin.shipment-delete', $shipping->id)}}"><i class="la la-trash"></i></a>
                                            </td>
                                        </tr>
                                    @empty
                                    @endforelse
...
<div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" enctype="multipart/form-data" id="saveShipment" class="was-validated">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLongTitle">Add Shipment</h4>
                ...
                <div class="modal-footer">
                    <input type="hidden" name="id" id="shipping_id" value="">
...
$(document).ready(function() {
    $('body').on('click','.btn-view',function(){
        ...
        let obj = $("#saveShipment");
        obj.find('.modal-title').text('Edit Shipment');
        obj.find('#shipping_id').val(id);
    ...
    $("#saveShipment").on("submit", function(e) {
        e.preventDefault();
        let self = $(this);
        let txt = self.text();
        let form = $("#saveShipment");
        ...
        if (name.trim()!='') {
            ...
            $.ajax({
                url : "{{ route('admin.shipping-type.store') }}",
                method: "POST",
                data : form.serialize(),
                dataType : 'json',
                headers: {
