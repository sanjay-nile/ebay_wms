                                ...
                                <tbody>
                                    ...
                                    @forelse($carrier_list as $carrier)
                                        <tr>
                                            ...
                                            <td>
                                                <button class="btn btn-view" data-id="{{ $carrier->id }}" data-status="{{ $carrier->status }}" data-name="{{ $carrier->name }}"><i class="la la-edit"></i></button>
...
<div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" enctype="multipart/form-data" id="saveCarrier" class="was-validated">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLongTitle">Add Carrier</h4>
                ...
                <div class="modal-footer">
                    <input type="hidden" name="id" id="carrier_id" value="">
                    <button type="button" class="btn btn-outline-secondary btn-sm action" data-dismiss="modal">Close</button>
                    <button type="submit" name="save" id="save-carrier" class="btn btn-outline-primary btn-sm btn-action">Save</button>
                </div>
            </div>
        </form>
    </div>
...
$(document).ready(function() {
    ...
    $("#saveCarrier").on("submit", function(e) {
        e.preventDefault();
        let self = $(this);
        let txt = self.text();
        let form = $("#saveCarrier");
