...
<head>
    ...
    <link href="https://db.onlinewebfonts.com/c/2173dceabf0710a335c0bca178efc162?family=Maison+Neue" rel="stylesheet">
    ...
    <style type="text/css"> 
        ...
        .QRcode .content ul li {padding-bottom: 5px; font-size: 18px; font-weight: bold; color: #263238; }
        .QRcode .content ul li span {font-size: 18px; font-weight: 500; color: #000; }
        .QRcode .content ul li:last-child{padding-bottom: 0px;}
        .QRcode-container{border: 1px solid #ccc; border-radius: 0px; padding: 20px; width:397px;height: 567px;}
        ...
        .invoice-content-info.content h3 {font-size: 20px; font-weight: 600; padding: 0;margin-bottom: 1rem; color: #263238; }
        .invoice-customer-name {font-size: 14px; font-weight: bold; color: #263238; text-align:right; }
        .invoice-content-info.content p {font-size: 20px; font-weight: 600; padding: 0;margin-top: 1rem; color: #263238; text-align: center;}
    </style>
</head>
        ...
        <div class="container">
            <div class="QRcode-container">
                <div class="QRcode-header">
                    <table align="center" cellpadding="0" cellspacing="0" width="100%"  style="font-family: 'Maison Neue';">
                        <tbody> 
                        <tr>
                            <td style="vertical-align:top;border-bottom: 1px solid #cccccc;padding: 0px 0 15px 0;">
                                <table align="center" cellpadding="0" cellspacing="0" width="100%">
                                    <tr>
                                        ...
                                        <td>
                                            <div class="invoice-QRcode-info" style=" width: 40%; text-align: end; margin-left: auto; ">
                                                @php
                                    ...
                                    <tr>
                                        <td>
                                            <div class="invoice-content-info content">
                                                <h3 style="padding-top: 10px;">Packing List</h3>
                                                <ul>
                                                    <li> To: <span>{!! $order_data_by_id['ship_to_name'] ?? '' !!}</span></li>
                                                    {{-- <li> Phone: <span>{!! $order_data_by_id['ship_to_phone'] ?? '' !!}</span></li> --}}
                                                    <li> Address : <span> {!! $order_data_by_id['ship_to_address_1'] ?? '' !!}</span> <span>{!! $order_data_by_id['ship_to_address_2'] ?? '' !!}</span> <span> {!! $order_data_by_id['ship_to_city'] ?? '' !!}</span> <span> {!! $order_data_by_id['ship_to_state'] ?? '' !!}</span> <span> {!! $order_data_by_id['ship_to_zip'] ?? '' !!}</span> <span> {!! $order_data_by_id['ship_to_country'] ?? '' !!}</span></li>
                                                    <li> eBay Order ID: <span> {!! $order_data_by_id['order_number'] ?? '' !!}</span></li>
                                                    <li> eBay Item ID: <span> {!! $order_data_by_id['legacyItemId'] ?? '' !!}</span></li>
                                                    <li> Item SKU: <span> {!! $order_data_by_id['scan_i_package_id'] ?? '' !!}</span></li>
                                                    <li> Weight: <span> {!! $order_data_by_id['weight'] ?? '' !!}</span></li>
                                                    <li> Qty: <span> {!! $order_data_by_id['item_quantity'] ?? '' !!}</span></li>
                                                    <li> Price: <span> {!! $order_data_by_id['item_price'] ?? '' !!}</span></li>
                                                    <li> eBay listing description: <span> {!! $order_data_by_id['item_title'] ?? '' !!}</span></li>
                                                </ul>
                                                <p style="border-top: 1px solid #cccccc; padding: 10px 0 0 0;">Thank You For Your Purchase</p>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
